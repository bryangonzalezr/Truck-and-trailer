...
public:
    ...
    Client* select_next_client(Instance& instance, Route& route) {
        ...
        // cout<<"all clients checked"<<endl;
        sort(sorted_clients.begin(), sorted_clients.end());

        // cout << "Sorted clients: " << endl;
        // for (auto client : sorted_clients) {
        //     cout << "Client: " << client.second->number << " Distance: " << client.first << endl;
        // }
        ...
        if (!sorted_clients.empty()) {
            ...
            //If empty, prioritize non truck customers
    ...
    void simple_greedy(Instance& instance) {
        ...
        for (int i = 0; i < instance.N_trucks; ++i) {
            // cout << "Creating route " << i << endl<<endl;
            ...
            while (route.current_trailer_capacity + route.current_truck_capacity > 0) {
                // cout << "Current location: "<< (route.clients.empty()? 0 : route.clients.back()->number) << endl<<endl;
                ...
                // cout << "Trailer location: " << (route.trailer_location==nullptr? -1: route.trailer_location->number) << endl;
                // Select the nearest client
                ...
                if (nearest_client == nullptr) {
                    if(!route.trailer_attached && !route.clients.empty() && route.trailer_location!=nullptr){
                        // cout<<endl;
                        // cout << "Going to trailer location" << endl;
                        // cout<<endl;

                        route.add_client(route.trailer_location);
                    }else{
                        break;;
                    }
                }else{
                    // cout <<endl;
                    // cout <<"Going to " << nearest_client->number << endl;
                    // cout<<endl;

                    route.add_client(nearest_client);
            ...
            // cout<<endl;
            route.add_client(&instance.deposit);
...
int main(int argc, char* argv[]) {
    ...
    best_solution.exportSolution(instance,"/");
